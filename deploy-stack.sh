...
build_and_push_images() {
    ...
    # Update API endpoint in script.js before building
    API_ENDPOINT="http://$NODE_IP:30080"
    log_info "Configuring UI for API endpoint: $API_ENDPOINT"
    sed -i "s|const API_BASE_URL = .*|const API_BASE_URL = '$API_ENDPOINT';|g" ui/script.js
    
    # Build FastAPI image
    log_info "Building FastAPI image..."
    docker build -f docker/api.Dockerfile -t $ECR_REPOSITORY_API:$IMAGE_TAG .
    ...
    docker build -f docker/ui.Dockerfile -t $ECR_REPOSITORY_UI:$IMAGE_TAG .
...
deploy_fastapi() {
    ...
    cp k8s/k8s-deployment-api.yaml /tmp/k8s-deployment-api.yaml
    
    # Update image in deployment manifest
    sed -i "s|image: .*amazonaws.com/churn-prediction-api:.*|image: $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/$ECR_REPOSITORY_API:$IMAGE_TAG|g" /tmp/k8s-deployment-api.yaml
    
    # Apply deployment
    kubectl apply -f /tmp/k8s-deployment-api.yaml --namespace=$K8S_NAMESPACE
...
deploy_ui() {
    ...
    # Create a temporary deployment file with updated image
    cp k8s/k8s-deployment-ui.yaml /tmp/k8s-deployment-ui.yaml
    
    # Update image in deployment manifest
    sed -i "s|image: .*amazonaws.com/churn-prediction-ui:.*|image: $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/$ECR_REPOSITORY_UI:$IMAGE_TAG|g" /tmp/k8s-deployment-ui.yaml
    
    # Apply deployment
    kubectl apply -f /tmp/k8s-deployment-ui.yaml --namespace=$K8S_NAMESPACE
...
deploy_monitoring() {
    ...
    if [ -f "observability/monitoring.yaml" ]; then
        kubectl apply -f observability/monitoring.yaml --namespace=$K8S_NAMESPACE
        log_success "Monitoring stack deployed"
    else
        log_warning "observability/monitoring.yaml not found, skipping monitoring deployment"
    fi
}
